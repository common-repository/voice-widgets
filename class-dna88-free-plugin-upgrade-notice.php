...
if( !class_exists('DNA88VoiceWidgetsPluginUpgradeToProNotice') )
{
	class DNA88VoiceWidgetsPluginUpgradeToProNotice {
		...
		function func_show_upgrade_link_with_action_links( $links ){
			...
			$links = array_merge( $links, array(
				'<a title="'.$this->link_text.'" class="'.$this->link_class.'" style="background:#0000ff; padding: 2px 5px;font-weight: bold; color: '.$this->link_color.';" href="' . esc_url( $this->upgrade_link ) . '" target="'.$this->link_target.'">' . __( $this->link_text, 'voice-widgets' ) . '</a>'
			) );
		...
		function func_show_upgrade_link_with_meta_links( $links, $file )
		{
			...
			if ( strpos( $file, "$this->plugin_main_file" ) !== false ) {
				...
				$new_links = array(
					'<a class="'.$this->link_class.'" style="background:#0000ff; padding: 2px 5px;font-weight: bold; color: '.$this->link_color.';" href="' . esc_url( $this->upgrade_link ) . '" title="'.$this->link_text.'" target="'.$this->link_target.'">' . __( $this->link_text, 'voice-widgets' ) . '</a>'
				);
